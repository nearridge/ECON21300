...
edited %>%
  ...
  kable(caption = "2018 Percent of Questions Answered Correctly")
...
edited %>%
  ...
  mutate(qno = glue::glue("Q {qno}")) %>%
  pivot_wider(id_cols = Round, names_from = qno, values_from = percent) %>%
  kable(caption = "2019 Percent of Questions Answered Correctly")
...
edited %>%
  ...
  labs(title = "Percent of questions correctly answered", x = "Question Number", y = "% Correctly Answered")
...
bind_cols(home2018, home2019) %>%
  ...
  kable("latex", longtable = T, booktabs = T, caption = "Longtable") %>%
