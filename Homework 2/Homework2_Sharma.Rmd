...
fittingd <- randomness %>%
  ...
  select(-education, -race_ethnicity) %>%
  drop_na(bmi)
...
fit1 <- lm(hours ~ treatment, data = fittingd)
fit2 <- lm(hours ~ treatment + bmi, data = fittingd)
fit3 <- lm(hours ~ treatment + bmi + hispanic + white, data = fittingd)
fit4 <- lm(hours ~ treatment + bmi + hispanic + white + community_center, data = fittingd)
