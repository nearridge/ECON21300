...
final <- fit4 %>%
  augment() %>%
  group_by(treatment) %>%
  summarize(count = n(), mean(.fitted), sd(.fitted), se = sd(.fitted)/sqrt(n())) 
...
After all this, the average treatment effect calculated after adjusting for errors in randomness and corresponding covariates is **`r round(final %>% extract(2, 3) - final %>% extract(1, 3), digits = 4)`** and the standard error on that treatment effect is **`r round(abs(final %>% extract(2, 5) - final %>% extract(1, 5)), digits = 4)`**.
