...
This super complicated dataset ended up being way too overcomplicated and sort of a waste of time. The demographic data was nice to have for sorting and exploration, but served little to no purpose in building my models as I did not end up comparing Illinois to any other state. 
...
# lm.weights <- lapply(seq(-13, 14, length.out = 28), lm.weight.function)
# 
# 
# regres1 <- model(apr_highpop %>% index_by(date) %>% summarize(deaths = sum(deaths)), lm = ETS(log(deaths) ~ error("A") + trend("N") + season("M")))
# regres2 <- model(apr_highpop %>% filter(!(is.na(m50))), lm = TSLM(log(deaths) ~ cases + `socialdistancing?`*date + m50 + at_icu_cap)) 
# 
# forecast_newdata <- tibble(date = seq(ymd("2020-04-15"), ymd("2020-05-31"), by = "1 day"))
#   #tibble(date = seq(ymd("2020-04-15"), ymd("2020-05-31"), by = "1 day"), `socialdistancing?` = rep(1,47))
# 
# forecast_lowpop <- augment(regres2) %>% forecast(forecast_newdata) %>% fortify(level = 80)
# 
# #regres3 <- lm(log(deaths) ~ date, apr_highpop %>% as_tibble() %>% group_by(date) %>% summarize(deaths = sum(deaths)), weights = unlist(lm.weights))
# #regres4 <- lm(log(deaths) ~ date, apr_highpop %>% as_tibble() %>% group_by(date) %>% summarize(deaths = sum(deaths)))
...
apr_highpop_experiment <- full %>%
  ...
  filter(deaths != 0, county %in% highpop) %>%
  group_by(date) %>%
  summarize(cases = sum(cases), deaths = sum(deaths), mean_m50 = mean(m50, na.rm = TRUE), bedsICU = sum(bedsIntensiveCare), icucap = mean(at_icu_cap)) %>%
  mutate(at_icu_cap = if_else(cases > bedsICU, 1, 0), 
         `socialdistancing?` = if_else(date > as.Date("2020-03-20"), 1, 0)) %>%
  as_tsibble(index = date)

regres2_experiment <- model(apr_highpop_experiment %>% filter(!(is.na(mean_m50))), lm = TSLM(log(deaths) ~ cases + `socialdistancing?`*date  + icucap))

forecast_newdata <- tibble(date = seq(ymd("2020-04-15"), ymd("2020-05-31"), by = "1 day"), `socialdistancing?` = rep(1,47))

forecast_highpop <- regres2_experiment %>% forecast(forecast_newdata) %>% fortify(level = 80)
...
model_cases <- nls(count ~ SSlogis(day, Asym, xmid, scal), versus %>% filter(var == "cases"))
forecast_cases <- model_cases %>% predict(newdata = tibble(day = seq(0,92)))

ggplot(augment(model_cases), mapping = aes(x = day)) +
  geom_point(aes(y = count, color = "Count")) +
  geom_point(aes(y = .fitted, color = "Fitted")) +
  geom_line(il %>% filter(var == "cases"), mapping = aes(x = day, y = count, color = "Illinois"), size = .75)
...
ggplot() +
  geom_line(versus %>% filter(var == "cases"), mapping = aes(x = day, y = count, group = interaction(countries, var), color = countries), alpha = 0.4) +
  geom_smooth(versus %>% filter(var == "cases"), method = "loess", mapping = aes(x = day, y = count), alpha = 0.4, level = 0.8) +
  geom_line(il %>% filter(var == "cases"), mapping = aes(x = day, y = count, color = "Illinois"), size = .75) +
  xlim(0, 51)
