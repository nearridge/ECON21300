...
Through my exploratory data analysis, I discovered a meaningful difference in the progression along the curve between high-population counties in Illinois and low-population counties in Illinois. Illinois is unique as it has several counties with extremely high populations relative to the rest of the country, several counties with moderately higher populations, and then medium to low population counties. What I found was that the progression along the curve, both in terms of cases and deaths, was further in high-population counties than in low population counties. Thus, I create a total of four models: two models to predict deaths in high vs low population counties in each of the two time horizons we are required to predict. 

### April Highpop
```{r}
highpop <- c("Cook", "DuPage","Lake", "Will", "Kane", "McHenry", "Winnebago", 
             "Madison", "St. Clair", "Champaign", "Sangamon", "Peoria", "McLean", 
             "Rock Island", "Tazewell", "Kendall", "Kankakee", "LaSalle", "Macon", "DeKalb")
...
apr <- full %>%
  select(date, county, cases, deaths, delta_cases, delta_deaths, cases_rate, 
         ...
         bedsTotalCONAuthorizedBeds, m50, `socialdistancing?`, overcrowding, at_icu_cap, 
         at_hospital_cap) %>%
  filter(deaths != 0) %>%
  as_tsibble(index = date, key = county)

regres2 <- model(apr %>% filter(!(is.na(m50))), lm = TSLM(log(deaths) ~ cases + date*`socialdistancing?`*m50 + at_icu_cap))
...
```
```{r}
regres1 <- model(apr, lm = TSLM(log(deaths) ~ date*`socialdistancing?` + at_icu_cap))

regres1 %>% filter(county == "Cook") %>% report()
ggplot(augment(regres1 %>% filter(county == "Cook")), aes(x = date)) +
  geom_line(aes(y = deaths, color = "Deaths")) +
  geom_line(aes(y = .fitted, color = "Fitted from Model"))
```
...
```{r message = FALSE}
country <- c("Russia", "Netherlands", "Switzerland", "Belgium", "France")
...
ggplot() +
  geom_line(versus %>% filter(var == "deaths"), mapping = aes(x = day, y = count, group = interaction(countries, var), color = countries), alpha = 0.4) +
  geom_point(versus %>% filter(day %% 4 == 2, var == "deaths"), mapping = aes(x = day, y = count, group = interaction(countries, var), color = countries, shape = var), alpha = 0.4) +
  geom_line(il %>% filter(var == "deaths"), mapping = aes(x = day, y = count, group = var, color = "Illinois"), size = .75) +
  geom_point(il %>% filter(day %% 4 == 0, var == "deaths"), mapping = aes(x = day, y = count, group = var, color = "Illinois", shape = var), size = 2) +
  scale_y_continuous(labels = scales::comma)
